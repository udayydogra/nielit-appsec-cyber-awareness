...
authRouter.post('/login', ah(async (req, res) => {
  ...
  const user = await one<{ id: string; email: string; password_hash: string; display_name: string; locale: string }>(
    `SELECT id, email, password_hash, display_name, locale FROM users WHERE email = $1`,
    [email.toLowerCase()],
  );
